...
def sortear_acao_positiva_custo_beneficio(custo_beneficio):
   global APs_custos
   index_resultado = -1
   while index_resultado == -1: # enquanto não encontrou o resultado
...
def sortear_acao_positiva(tipo_roleta, best, custo_beneficio):
   ...
   if (tipo_roleta == 'prioridade'):
      ...
      print("Tipo de roleta inválida.".format(tipo_roleta))      
...
#print(qtde_vezes_AP_executada)
   ...
   if prefixo_tipo_ciclo == "tempo_acoes":
      ...
      print("Para o próximo ciclo devem ser executadas {} ações. Valor escolhido entre {} e {}".format(qtde_APs_executadas_para_recalcular_modelo, math.ceil(qtde_APs_executadas_para_recalcular_modelo_max / 2), qtde_APs_executadas_para_recalcular_modelo_max))
   ...
   while total_recurso_compartilhado > 0:
      ...
      # se terminou as quantidades disponíveis da AP, zera o seu valor para não ser mais escolhido na roleta
      ...
      #print("Ação executada: {}. Unidades monetárias: {}. RC: {}. Roleta {} {}".format(acao_positiva, qtde, total_recurso_compartilhado, tipo_roleta, prefixo))
      if (qtde_ciclos_recalcular_modelo > 1) and \
         ...
         if (qtde_APs_executadas_para_recalcular_modelo > 0):
            print("Recálculo após {} APs executadas".format(qtde_APs_executadas_ciclo_atual))
         else:
            print("Recálculo após {} recursos consumidos".format(qtde_recurso_consumido_ciclo_atual))

         count_ciclos+=1
         if (count_ciclos > qtde_ciclos_recalcular_modelo):
            print("Fim da simulação. Já foram executados {} ciclos. Qtde ciclos a ser executados: {}.".format(count_ciclos-1, qtde_ciclos_recalcular_modelo))
            break
         ...
         if  tipo_roleta_src == "todas_roletas":
            tipo_roleta = proxima_roleta(tipo_roleta) # retorna a próxima roleta a ser utilizada
         ...
         # se ainda só possui 1 AP, ajusta manualmente o seu valor
   ...
   # armazena dados do último ciclo
   utilizacao_RCs = np.vstack((utilizacao_RCs, total_recurso_compartilhado))
   all_qtde_vezes_AP_executada = np.vstack((all_qtde_vezes_AP_executada, qtde_vezes_AP_executada))
...
def executar_simulacoes():
   ...
   # print("Iniciando todas roletas aleatórias...")
   ...
   print("Iniciando roleta aleatória...")
